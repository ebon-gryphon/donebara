...
当前版本默认保护 Git 仓库，也会保护全局 Skill、插件和 Codex CLI 配置；非 Git 目录可以通过 `.donebara.json` 显式启用。默认模式为 `warn`，它会展示报告，但不会阻止任务结束。
...
每次用户提示开始时，水豚验收官会同时记录 Git 脏文件内容状态和当时生效的项目规则。补丁修改与后续脚本生成的文件会在停止前合并检查；同一轮涉及多个受保护仓库时，一处仓库的成功命令不能替其他仓库提供完成证据。若本轮修改 `.donebara.json`，新规则从后续轮次生效，本轮仍使用提示开始时的规则，避免检查过程被同一轮改动意外关闭。

旧配置名 `.doneguard.json` 仍兼容；同一目录同时存在时优先读取 `.donebara.json`，不合并两份配置。
...
在 Git 仓库根目录创建 `.donebara.json`。也可以在非 Git 工程目录中创建该文件，将该目录显式纳入保护。最小配置如下。
...
- 在未配置 `.donebara.json` 的普通非 Git 目录编辑一般文件。
...
- 使用 `.donebara.json` 显式启用的非 Git 目录。
...
验证包含 Python 单元测试、Swift 报告删除与通知队列烟雾测试。通知测试覆盖“稍后”释放队列、展示回执匹配、事件先被消费的时序、详情与新通知隔离、损坏事件恢复，以及未确认展示时的聊天内回退；原有完成证据与 strict 续跑检查继续保留。
